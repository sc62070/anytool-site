...
export default function PopularTools() {
  ...
  const sorted = useMemo(() => {
    return [...tools].sort((a, b) => (views[b.slug] || 0) - (views[a.slug] || 0))
  }, [views])
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-24">
      ...
      <div className="text-center mb-16">
        ...
        <p className="text-gray-500 dark:text-gray-400 max-w-lg mx-auto text-lg">Ranked by usage. Updated in real time.</p>
      </div>

      <div className="space-y-3">
        {sorted.map((tool, i) => {
          const Icon = tool.icon
          const viewCount = views[tool.slug] || 0
          return (
            <Link key={tool.slug} to={`/tools/${tool.slug}`} className="group flex items-center gap-4 p-4 bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 hover:border-violet-300 dark:hover:border-violet-500/50 transition-all no-underline">
              ...
              <div className="text-right shrink-0">
                <div className="text-sm font-medium text-gray-500 dark:text-gray-400">{viewCount.toLocaleString()} views</div>
