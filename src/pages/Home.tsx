...
const NEW_SLUGS = new Set(['js-formatter','css-formatter','html-formatter','jwt-decoder','yaml-json','xml-formatter','cron-builder','number-base','mock-data','json-to-typescript','slug-generator','text-sorter','remove-duplicates','html-stripper','morse-code','ascii-art','typing-speed','placeholder-image','favicon-generator','color-palette','password-strength','rotate-pdf','compress-pdf','image-to-pdf','css-gradient','box-shadow','flexbox-playground','glassmorphism','contrast-checker','gpa-calculator','mortgage-calculator','scientific-calculator','fraction-calculator','roman-numerals','fibonacci','factorial','dns-lookup','http-status','user-agent','subnet-calculator'])
...
export default function Home() {
  ...
  const filteredTools = useMemo(() => {
    ...
    if (!search.trim() && activeCategory === 'All' && !favFilter) {
      result.sort((a, b) => (views[b.slug] || 0) - (views[a.slug] || 0))
    }

    return result
  }, [search, activeCategory, favFilter, favorites, views])

  const recentTools = useMemo(() => {
    return recent.map(slug => tools.find(t => t.slug === slug)).filter(Boolean).slice(0, 5)
  }, [recent])
  ...
  return (
    <div>
        ...
        <div className="relative max-w-6xl mx-auto px-4 py-32">
          <div className="max-w-3xl">
            ...
            <div className="flex flex-wrap gap-4">
              <a href="#tools" className="group inline-flex items-center gap-2 bg-white text-gray-900 px-6 py-3 rounded-xl font-semibold hover:bg-gray-100 transition-all no-underline">
                Explore Tools
                <ArrowUpRight className="w-4 h-4 group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-transform" />
              </a>
              <Link to="/blog" className="inline-flex items-center gap-2 border border-white/20 text-white px-6 py-3 rounded-xl font-semibold hover:bg-white/5 transition-all no-underline">
                Read the Blog
              </Link>
            </div>
          </div>
          ...
          <div className="hidden lg:block absolute right-0 top-1/2 -translate-y-1/2">
            <div className="relative w-72 h-72">
              <div className="absolute top-0 right-0 bg-white/10 backdrop-blur-sm border border-white/10 rounded-2xl px-4 py-3 animate-bounce" style={{ animationDuration: '3s' }}>
                <div className="flex items-center gap-2"><Zap className="w-4 h-4 text-yellow-400" /><span className="text-sm text-white font-medium">Instant results</span></div>
              </div>
              <div className="absolute top-20 right-16 bg-white/10 backdrop-blur-sm border border-white/10 rounded-2xl px-4 py-3 animate-bounce" style={{ animationDuration: '4s', animationDelay: '0.5s' }}>
                <div className="flex items-center gap-2"><Shield className="w-4 h-4 text-emerald-400" /><span className="text-sm text-white font-medium">100% private</span></div>
              </div>
              <div className="absolute top-44 right-0 bg-white/10 backdrop-blur-sm border border-white/10 rounded-2xl px-4 py-3 animate-bounce" style={{ animationDuration: '3.5s', animationDelay: '1s' }}>
                <div className="flex items-center gap-2"><Globe className="w-4 h-4 text-violet-400" /><span className="text-sm text-white font-medium">Works everywhere</span></div>
              </div>
            </div>
          </div>
        </div>
      ...
      <section id="tools" className="max-w-6xl mx-auto px-4 py-24">
        ...
        {/* Recent Tools */}
        {recentTools.length > 0 && !search && activeCategory === 'All' && !favFilter && (
          <div className="mb-8">
            <div className="flex items-center gap-2 mb-3">
              <History className="w-4 h-4 text-gray-400" />
              <span className="text-sm font-medium text-gray-500 dark:text-gray-400">Recently used</span>
            </div>
            <div className="flex gap-2 overflow-x-auto pb-2">
              {recentTools.map(tool => {
                ...
                return (
                  <Link key={tool.slug} to={`/tools/${tool.slug}`} className="flex items-center gap-2 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl px-3 py-2 no-underline hover:border-violet-300 dark:hover:border-violet-500/50 transition-all shrink-0">
                    <Icon className="w-4 h-4 text-violet-500" />
                    <span className="text-sm text-gray-700 dark:text-gray-300 whitespace-nowrap">{tool.name}</span>
                  </Link>
                )
              })}
            </div>
        ...
        <div className="mb-8 space-y-4">
          ...
          <div className="flex flex-wrap justify-center gap-2">
            <button onClick={() => { setFavFilter(false); setActiveCategory('All') }} className={`px-4 py-1.5 rounded-lg text-sm font-medium transition-all ${!favFilter && activeCategory === 'All' ? 'bg-violet-600 text-white shadow-lg shadow-violet-500/25' : 'bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-gray-700 border border-gray-200 dark:border-gray-700'}`}>All</button>
            <button onClick={() => { setFavFilter(!favFilter); setActiveCategory('All') }} className={`px-4 py-1.5 rounded-lg text-sm font-medium transition-all flex items-center gap-1.5 ${favFilter ? 'bg-yellow-500 text-white shadow-lg shadow-yellow-500/25' : 'bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-gray-700 border border-gray-200 dark:border-gray-700'}`}>
              <Star className={`w-3.5 h-3.5 ${favFilter ? 'fill-white' : ''}`} /> Favorites
            </button>
            {categories.filter(c => c !== 'All').map((cat) => (
              <button key={cat} onClick={() => { setActiveCategory(cat); setFavFilter(false) }} className={`px-4 py-1.5 rounded-lg text-sm font-medium transition-all ${activeCategory === cat && !favFilter ? 'bg-violet-600 text-white shadow-lg shadow-violet-500/25' : 'bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-gray-700 border border-gray-200 dark:border-gray-700'}`}>{cat}</button>
            ))}
          </div>
        </div>
        ...
        {filteredTools.length === 0 ? (
          <div className="text-center py-16">
            <p className="text-gray-500 dark:text-gray-400 text-lg">{favFilter ? "No favorites yet. Click the star on any tool to add it." : `No tools found matching "${search}"`}</p>
            <button onClick={() => { setSearch(''); setActiveCategory('All'); setFavFilter(false) }} className="mt-3 text-violet-600 dark:text-violet-400 hover:text-violet-700 dark:hover:text-violet-300 text-sm font-medium">Clear filters</button>
          </div>
        ) : (
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredTools.map((tool) => {
              ...
              return (
                <Link key={tool.slug} to={`/tools/${tool.slug}`} className="group relative bg-white dark:bg-gray-900 rounded-2xl p-6 border border-gray-200 dark:border-gray-800 hover:border-violet-300 dark:hover:border-violet-500/50 transition-all duration-300 no-underline overflow-hidden">
                  ...
                  <div className="relative">
                    <div className="flex items-start justify-between mb-4">
                      <div className="flex items-center gap-2">
                        ...
                        {isNew && <span className="text-[10px] font-bold text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-500/10 px-1.5 py-0.5 rounded-md">NEW</span>}
                      </div>
                      <div className="flex items-center gap-1">
                        <button onClick={(e) => handleToggleFav(e, tool.slug)} className={`p-1.5 rounded-lg transition-colors ${isFav ? 'text-yellow-500 hover:text-yellow-600' : 'text-gray-300 dark:text-gray-600 hover:text-yellow-500'}`}>
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {blogPosts.slice(0, 3).map((post) => (
              <Link key={post.slug} to={`/blog/${post.slug}`} className="group block bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700/50 overflow-hidden hover:shadow-xl hover:shadow-gray-200/50 dark:hover:shadow-gray-900/50 hover:border-transparent transition-all duration-300 no-underline">
                <div className="relative h-48 overflow-hidden">
                  <img src={post.image} alt={post.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                  <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent" />
                  <div className="absolute bottom-4 left-4"><span className="bg-white/20 backdrop-blur-sm text-white text-xs font-medium px-2.5 py-1 rounded-full">{post.category}</span></div>
                </div>
                <div className="p-5">
          ...
          <Link to="/blog" className="md:hidden inline-flex items-center gap-2 text-sm font-semibold text-violet-600 dark:text-violet-400 mt-8">View All Posts <ArrowRight className="w-4 h-4" /></Link>
        </div>
      </section>

      {/* Newsletter */}
      <section className="max-w-4xl mx-auto px-4 py-16">
        <div className="relative p-8 md:p-12 rounded-2xl bg-gradient-to-br from-violet-600 to-fuchsia-600 overflow-hidden">
