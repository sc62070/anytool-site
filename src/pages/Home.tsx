        ...
        <div className="max-w-6xl mx-auto">
          <div className="flex items-center justify-between mb-8">
            <div>
              ...
              <p className="text-gray-600">Tips, guides, and insights about our tools.</p>
            </div>
            <Link to="/blog" className="hidden md:inline-flex items-center text-sm font-medium text-indigo-600 hover:text-indigo-700">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {blogPosts.slice(0, 3).map((post) => (
              <Link
                key={post.slug}
                to={`/blog/${post.slug}`}
                className="block bg-gray-50 rounded-xl p-6 border border-gray-200 hover:shadow-md hover:border-indigo-300 transition-all no-underline"
              >
                <div className="flex items-center gap-3 text-xs text-gray-500 mb-3">
                  <span className="bg-indigo-100 text-indigo-700 px-2 py-0.5 rounded-full font-medium">{post.category}</span>
          ...
          <Link to="/blog" className="md:hidden inline-flex items-center text-sm font-medium text-indigo-600 mt-6">
