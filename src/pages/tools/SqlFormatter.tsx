  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-12">
      <div className="flex items-center gap-3 mb-2">
        ...
        <h1 className="text-3xl font-bold text-gray-900 dark:text-gray-100">SQL Formatter</h1>
      </div>
      <p className="text-gray-600 dark:text-gray-400 mb-8">Format and beautify SQL queries for better readability.</p>

      <div className="grid md:grid-cols-2 gap-4">
        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Input SQL</label>
          <textarea
            value={input}
            onChange={(e) => setInput(e.target.value)}
            placeholder="SELECT id, name, email FROM users WHERE age > 18 ORDER BY name ASC"
            className="w-full h-80 p-3 border border-gray-300 dark:border-gray-600 rounded-xl font-mono text-sm resize-y"
          />
        </div>
        <div>
          ...
          <textarea readOnly value={output} className="w-full h-80 p-3 border border-gray-300 dark:border-gray-600 rounded-xl bg-gray-50 dark:bg-gray-700 font-mono text-sm resize-y" placeholder="Formatted SQL will appear here..." />
        </div>
      </div>
