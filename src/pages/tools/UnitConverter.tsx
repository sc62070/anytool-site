...
export default function UnitConverter() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-12">
      ...
      <p className="text-gray-600 mb-8">Convert between length, weight, temperature, speed, and data units.</p>
      ...
      <div className="bg-white rounded-xl p-6 border border-gray-200 space-y-4">
          ...
          <div className="flex flex-wrap gap-2">
            {Object.keys(categories).map(cat => (
              <button key={cat} onClick={() => { setCategory(cat as keyof typeof categories); setFromUnit(categories[cat as keyof typeof categories].units[0]); setToUnit(categories[cat as keyof typeof categories].units[1] || categories[cat as keyof typeof categories].units[0]) }}
                className={`px-4 py-2 rounded-lg text-sm font-medium ${category === cat ? 'bg-indigo-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>
                {cat}
              </button>
            ))}
          </div>
        ...
        <div className="grid grid-cols-[1fr,auto,1fr] gap-4 items-end">
          <div>
            ...
            <input type="number" value={value} onChange={(e) => setValue(e.target.value)} className="w-full p-3 border border-gray-300 rounded-xl mb-2" />
            <select value={fromUnit} onChange={(e) => setFromUnit(e.target.value)} className="w-full p-3 border border-gray-300 rounded-xl">
              {units.map(u => <option key={u} value={u}>{u}</option>)}
            </select>
          </div>

          <button onClick={swap} className="p-3 bg-indigo-100 rounded-xl hover:bg-indigo-200 mb-0.5">
            <ArrowRightLeft className="w-5 h-5 text-indigo-600" />
          </button>

          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">To</label>
            <input readOnly value={result} className="w-full p-3 border border-gray-300 rounded-xl bg-gray-50 mb-2" />
            <select value={toUnit} onChange={(e) => setToUnit(e.target.value)} className="w-full p-3 border border-gray-300 rounded-xl">
              {units.map(u => <option key={u} value={u}>{u}</option>)}
            </select>
          </div>
        </div>
