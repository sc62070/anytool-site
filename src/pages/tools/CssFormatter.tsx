    ...
    try {
      if (minify) {
        ...
        for (let i = 0; i < input.length; i++) {
          const c = input[i]
          if (c === '{') { result += ' {\n' + '  '.repeat(++indent) }
          else if (c === '}') { result = result.trimEnd() + '\n' + '  '.repeat(--indent) + '}\n' + '  '.repeat(indent) }
          else if (c === ';') { result += ';\n' + '  '.repeat(indent) }
          else { result += c }
        }
        setOutput(result.replace(/\n\s*\n/g, '\n').trim())
      }
    } catch { setOutput('Error formatting CSS') }
  }

  const copy = () => { navigator.clipboard.writeText(output); setCopied(true); setTimeout(() => setCopied(false), 2000) }

  return (
    <ToolLayout title="CSS Formatter" description="Format, beautify, or minify CSS code." icon={Paintbrush}>
      <div className="grid md:grid-cols-2 gap-4">
        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Input</label>
          <textarea value={input} onChange={e => setInput(e.target.value)} placeholder="Paste CSS code..." className="w-full h-72 p-4 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl text-sm font-mono resize-y focus:ring-2 focus:ring-violet-500 focus:border-violet-500 outline-none text-gray-900 dark:text-white placeholder:text-gray-400" />
        </div>
        <div>
