...
export default function XmlFormatter() {
  ...
  const format = (minify = false) => {
    if (minify) { setOutput(input.replace(/>\s+</g, '><').trim()); return }
    let result = '', indent = 0
  ...
  return (
    <ToolLayout title="XML Formatter" description="Format, beautify, or minify XML data." icon={FileCode}>
      <div className="grid md:grid-cols-2 gap-4">
        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Input</label>
          <textarea value={input} onChange={e => setInput(e.target.value)} placeholder="Paste XML..." className="w-full h-72 p-4 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl text-sm font-mono resize-y focus:ring-2 focus:ring-violet-500 focus:border-violet-500 outline-none text-gray-900 dark:text-white placeholder:text-gray-400" />
        </div>
        <div>
