...
export default function ImageResizer() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-12">
      ...
      <p className="text-gray-600 mb-8">Resize images to exact dimensions. All processing happens in your browser.</p>
      ...
      {!original ? (
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between bg-white rounded-xl p-4 border border-gray-200">
            <span className="text-sm text-gray-700">{original.name} ({originalDimensions.w} x {originalDimensions.h})</span>
          ...
          <div className="bg-white rounded-xl p-6 border border-gray-200">
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input type="number" value={width} onChange={(e) => setWidth(Number(e.target.value))} className="w-full p-3 border border-gray-300 rounded-lg" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Height (px)</label>
                <input type="number" value={height} onChange={(e) => setHeight(Number(e.target.value))} className="w-full p-3 border border-gray-300 rounded-lg" />
              </div>
            </div>
            <div className="flex gap-2 mt-4">
              {[25, 50, 75, 100, 150, 200].map((pct) => (
                <button key={pct} onClick={() => { setWidth(Math.round(originalDimensions.w * pct / 100)); setHeight(Math.round(originalDimensions.h * pct / 100)) }} className="px-3 py-1 text-sm bg-gray-100 rounded-lg hover:bg-indigo-100">{pct}%</button>
              ))}
            </div>
          </div>
          ...
          {preview && (
            <div className="bg-white rounded-xl p-6 border border-gray-200 text-center">
              ...
              <p className="text-sm text-gray-500 mb-4">New size: {width} x {height} px</p>
